...
use crate::ble::services::battery::BatteryService;
use crate::ble::services::configuration::{BLE_NAME, ThingyConfigurationService};
use crate::ble::services::environment::{
    TesGas, TesPressure, TesTemperature, ThingyEnvironmentService,
};
use crate::ble::services::motion::{ThingyMotionService, TmsGravity};
use crate::ble::services::sound::ThingySoundService;
use crate::ble::services::ui::ThingyUiService;
...
async fn env_notifier(conn: &GattConnection<'_, '_, DefaultPacketPool>, server: &Server<'_>) {
    ...
    let pressure = &server.env.pressure;
    let temperature = &server.env.temperature;

    let mut rx_co2 = sense_co2::get_sensor_receiver().unwrap();
    let mut rx_pa = sense_pa::get_sensor_receiver().unwrap();

    loop {
        let co2 = rx_co2.changed().await;

        if let Err(e) = gas.notify(conn, &TesGas::new(co2.co2)).await {
            warn!("[gatt] notification error: {}", e);
        }
        if let Err(e) = humidity.notify(conn, &(co2.humidity as u8)).await {
            warn!("[gatt] notification error: {}", e);
        }

        let m_hpa = rx_pa.changed().await;

        if let Err(e) = pressure
            .notify(conn, &TesPressure::new(m_hpa.hpa, m_hpa.hpa_dec))
            .await
        {
            warn!("[gatt] notification error: {}", e);
        }
        ...
        if let Err(e) = temperature
            .notify(conn, &TesTemperature::new(m_hpa.temp_f, m_hpa.temp_f_dec))
            .await
        {
            warn!("[gatt] notification error: {}", e);
        }
